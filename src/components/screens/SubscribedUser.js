              ...
              <div className="home-card" key={item._id}>
                ...
                <div className="card-content">
                  <div className="likes">
                    ...
                    <span>{item.likes.length} likes</span></div>
                  <h6>{item.title}</h6>
                  <p>{item.body}</p>
                  {
                    item.comments.map(record=>{
                      return(
                        <div className='posted-comment' key={record._id}><b>{record.postedBy.name}</b>{record.text}</div>
                      )
                    })
                  }
                  <form onSubmit={(e)=>{makeComment(e,e.target[0].value,item._id)
                  }}>
